...
docker pull $IMAGE_TAG > out.log 2> err.log
touch ./customSpec.json
docker run -it -v ./customSpec.json:/customSpec.json $IMAGE_TAG sh -c '/node-template build-spec --disable-default-bootnode --chain local > customSpec.json' 1>&2
# chainSpec=$(cat customSpec.json | sed 1d) #get file content and remove first line (has some unwanted output)
chainSpec=$(cat customSpec.json) # | jq)
...
#build raw chainSpec
touch ./customSpecRaw.json
docker run -it -v ./customSpecRaw.json:/customSpecRaw.json -v $(pwd)/customSpec.json:/customSpec.json $IMAGE_TAG sh -c '/node-template build-spec --chain=/customSpec.json --raw --disable-default-bootnode > customSpecRaw.json'  1>&2
