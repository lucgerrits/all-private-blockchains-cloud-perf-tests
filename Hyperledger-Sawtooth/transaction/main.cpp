...
struct Arg_Options
{
    ...
    std::string ipfs_endpoint;
    bool isverbose = false;

    std::string arg_key = "";
    int arg_value = -1;

    std::string arg_accident_ID =""; // IPFS hash of data
    std::string arg_signature = ""; // Signature of the IoT dvice => proof it takes part of the echosystem 
    std::string arg_dataPublicKey = ""; // Public Key a for the decryption of Data (that sent to the IPFS system)
...
parse(int argc, char *argv[], Arg_Options &arg_options)
{
    try
    {
        ...
        options
            ...
            ("url_ipfs", "IPFS REST API endpoint.", cxxopts::value<std::string>()) //Is different for eth ?
        ...
        options.add_options("Cartp:crash")
            ...
            ("accident_ID", "accident_ID value", cxxopts::value<std::string>())
            ("signature", "signature value", cxxopts::value<std::string>())
            ("dataPublicKey", "dataPublicKey value", cxxopts::value<std::string>())
            ("file_name", "file_name value", cxxopts::value<std::string>())
        ...
        //cartp stuff
        if (result.count("accident_ID"))
        {
            arg_options.arg_accident_ID = result["accident_ID"].as<std::string>();
        }
        if (result.count("signature"))
        {
            arg_options.arg_signature = result["signature"].as<std::string>();
        }
        if (result.count("dataPublicKey"))
        {
            arg_options.arg_dataPublicKey = result["dataPublicKey"].as<std::string>();
        }
        ...
        if (result.count("file_name"))
        {
            arg_options.arg_file_name = result["file_name"].as<std::string>();
        }
        ...
        // }
        if (result.count("url_ipfs"))
        {
            arg_options.ipfs_endpoint = result["url_ipfs"].as<std::string>();
        }
        //eth stuff
...
int main(int argc, char **argv)
{
    ...
    if (!strcmp(options.arg_mode.c_str(), "test"))
    {
        ...
        message = "test";
          ...
          {
        //    unsigned char message_hash_char[HASH_SHA256_SIZE+2];
        //    std::cout << " ########  Hash test with file for IPFS ##########" << std::endl;

        //    std::ifstream fileInput("foo.txt");
        //    std::string strInput; //((std::istreambuf_iterator<char>(fileInput)), std::istreambuf_iterator<char>());

        //     fileInput.seekg(0, std::ios::end);   
        //     strInput.reserve(fileInput.tellg());
        //     fileInput.seekg(0, std::ios::beg);

        //     strInput.assign((std::istreambuf_iterator<char>(fileInput)),
        //     std::istreambuf_iterator<char>());


        //    std::string hash_type("12");
        //    std::string hash_size("20"); 
        // /* IPFS HASH creation*/
        //     std::cout << "Message test is ok." << std::endl;
        //     //std::cout << "->Using:" << message << std::endl;
        //     message_hash_str = sha256IPFSData(strInput);
        //     std::cout << "SHA256 test is ok." << std::endl;
        //     std::string final_hash = hash_type + hash_size + message_hash_str;
        //     //CHECK(final_hash == "12202c26b46b68ffc68ff99b453c1d30413413422d706483bfa0f98a5e886266e7ae");

            

        //     HexStrToUchar(message_hash_char, final_hash.c_str(), (size_t)HASH_SHA256_SIZE+2);

        //     std::cout << "->MSG SHA256:" << message_hash_str << std::endl;
        //     std::cout << "->FINEAL MSG SHA256:" << final_hash << std::endl;
            
        //     std::vector<unsigned char> v(message_hash_char, message_hash_char + 34);
            
        //     std::string base58_hash = EncodeBase58(v.data(), v.data() + v.size());
        //     std::cout << "->FINEAL base58_hash :" << base58_hash << std::endl;

            std::string hash_type("12");
            std::string hash_size("20"); 
            std::cout << " *********** Test with file ************" << std::endl;

            unsigned char message_hash_char_file[HASH_SHA256_SIZE+2];

            message_hash_str = sha256IPFSDataFile("foo.txt", "a");


            std::cout << "SHA256 test is ok." << std::endl;
            std::string final_hash_file = hash_type + hash_size + message_hash_str;

            HexStrToUchar(message_hash_char_file, final_hash_file.c_str(), (size_t)HASH_SHA256_SIZE+2);

            std::cout << "->MSG FILE SHA256:" << message_hash_str << std::endl;
            std::cout << "->FINAL FILE MSG SHA256:" << final_hash_file << std::endl;
            
            std::vector<unsigned char> v2(message_hash_char_file, message_hash_char_file + 34);
            
            std::string base58_hash_file = EncodeBase58(v2.data(), v2.data() + v2.size());
            std::cout << "->FINAL FILE base58_hash :" << base58_hash_file << std::endl;
        ...
        {
           unsigned char message_hash_char[HASH_SHA256_SIZE+2];
           std::cout << " ########  Hash test for IPFS ##########" << std::endl;
        ...
        /* IPFS HASH creation*/
            std::cout << "Message test is ok." << std::endl;
            //std::cout << "->Using:" << message << std::endl;
            message_hash_str = sha256IPFSData(input);
            std::cout << "SHA256 test is ok." << std::endl;
            std::string final_hash = hash_type + hash_size + message_hash_str;
            //CHECK(final_hash == "12202c26b46b68ffc68ff99b453c1d30413413422d706483bfa0f98a5e886266e7ae");
    ...
    else if (!strcmp(options.arg_mode.c_str(), "cartp"))
    {
        ...
        else if (!strcmp(options.arg_command.c_str(), "crash"))
        {
            ...
            if ( options.arg_accident_ID.empty() ||
                options.arg_signature.empty() ||
                options.arg_dataPublicKey.empty()
                // options.arg_date_of_the_accident.empty() ||
            ...
            {
                std::cout << "ERROR: ";
                std::cout << "accident_ID ";
                std::cout << "signature ";
                std::cout << "dataPublicKey ";
            ...
            payload["owner_id"] = TnxKeys.pubKey;
            payload["car_id"] = BatchKeys.pubKey;
            payload["accident_ID"] = options.arg_accident_ID;
            payload["signature"] = options.arg_signature;
            payload["dataPublicKey"] = options.arg_dataPublicKey;
    ...
    else if (!strcmp(options.arg_mode.c_str(), "measurments"))
    {

        std::string inputFileName =  options.arg_file_name.c_str();
        
        std::ifstream t(inputFileName);
        std::string message_str((std::istreambuf_iterator<char>(t)),
                            std::istreambuf_iterator<char>());

        boost::erase_all(inputFileName, "dataSet");
        std::string fileToWriteMeasurements = "dataSet/measurements/hash"; // + inputFileName;
        std::string str = fileToWriteMeasurements + inputFileName;
        //std::cout << str << std::endl;
        // measurementsFile.open(str, std::ofstream::out | std::ofstream::app);

        //auto startIoT = std::chrono::steady_clock::now();
        //std::ofstream myfile;
        //myfile.open("Time_Sha512_data2MB.txt", std::ofstream::out | std::ofstream::app);
        //myfile.close();


        //auto startJsonCbor = std::chrono::steady_clock::now();
        ...
        mydata.set_data(message_str);

        std::string mydata_str = mydata.SerializeAsString();
        ...
        if (options.isverbose)
            std::cout << " ########## In latency measurements ############" << std::endl;
        ...
        if (!strcmp(options.arg_command.c_str(), "crash"))
        {
             //json payload;
            //set payload for the command crash
            if (options.arg_dataPublicKey.empty())
            {
                std::cout << "ERROR: ";
                std::cout << "dataDecrKey ";
                std::cout << "is required." << std::endl;
                exit(1);
            }

            if(options.arg_file_name.empty())
            {
                std::cout << "ERROR: ";
                std::cout << "file_name ";
                std::cout << "is required." << std::endl;
            ...
            {
                if (options.isverbose)
                    std::cout << "*** HASH Creation ***" << std::endl;

                //std::string fileName = options.arg_file_name.c_str();
                //std::cout << "File Name : " <<  fileName << std::endl;



                std::string ipfsHash = creatIpfsHash(mydata_str, str);
           
                // Gnereate the Hash
                payload["accident_ID"] = ipfsHash;
                //std::cout << " Hash : " << ipfsHash << std::endl;


                
            }

            if (options.arg_signature.empty())
            {
                if (options.isverbose)
                    std::cout << "*** Signature Creation ***" << std::endl;
                // Gnereate the Signature

                GenerateKeyPair(ctx, SignatureKeys.privateKey, SignatureKeys.privKey, SignatureKeys.publicKey, SignatureKeys.publicKey_serilized, SignatureKeys.pubKey, options.isverbose);
        
                unsigned char message_hash_char_sig[HASH_SHA256_SIZE];
                unsigned char signature_serilized_sig[SIGNATURE_SERILIZED_SIZE];

                std::string msg_hash_str = "";
                std::string msg_sig = SignatureKeys.pubKey;
                std::string signature_serilized_str_hash = "";
                SECP256K1_API::secp256k1_ecdsa_signature Sig;
                msg_hash_str = sha256Data(msg_sig);
                HexStrToUchar(message_hash_char_sig, msg_hash_str.c_str(), (size_t)HASH_SHA256_SIZE);
                CHECK(SECP256K1_API::secp256k1_ecdsa_sign(ctx, &Sig, message_hash_char_sig /*TnxKeys.publicKey_serilized*/, SignatureKeys.privateKey, NULL, NULL) == 1); //make signature
                CHECK(SECP256K1_API::secp256k1_ecdsa_signature_serialize_compact(ctx, signature_serilized_sig, &Sig) == 1);
                signature_serilized_str_hash = UcharToHexStr(signature_serilized_sig, SIGNATURE_SERILIZED_SIZE);
                
                //std::cout << "Signature : " <<  signature_serilized_str_hash << std::endl;
                payload["signature"] = signature_serilized_str_hash;
            }
        ...
        {
            //###################### Send data to IPFS REST API ###########################
            
            if (!strcmp(options.ipfs_endpoint.c_str(), ""))
            {
                std::cout << "WARNING: ipfs_url is required to send the transaction." << std::endl;
            }
            else{

                //std::string endpoint_address = "http://134.59.230.100:5000/" + SignatureKeys.pubKey + "/upload";

                std::string endpoint_address = options.ipfs_endpoint + '/' + SignatureKeys.pubKey + "/upload";
                //std::cout << endpoint_address << std::endl;

                //CHECK(sendData(mydata_str, endpoint_address, options.isverbose) == 1);
            
            }  
        
            
            //std::cout << "Sending batch list to:" << options.api_endpoint << "" << std::endl;
            //std::cout << "Batch is sent, the measure ment is finished" << std::endl;

            //###################### Send Hash to Sawtooth ###########################


               // CHECK(sendData(data_string, options.api_endpoint, options.isverbose) == 1);


            //auto endIoT =  std::chrono::steady_clock::now();
            
            //std::cout << std::chrono::duration_cast<std::chrono::microseconds>(endIoT - startIoT).count() << std::endl;

            //measurementsFile << std::chrono::duration_cast<std::chrono::microseconds>(endIoT - startIoT).count() << std::endl;
    ...
    else if (!strcmp(options.arg_mode.c_str(), "genSigAndKeys"))
    {
        /*******************************************************************************************************************/
        /*######################################## TEST KEY GENERATION AND SIGNATURE ######################################*/ 

        std::cout << "***Generating keys***" << std::endl;

        std::ofstream publickeyFile, privatekeyFile, signatureFile;
        int i;
        publickeyFile.open("publickeys.pub");
        privatekeyFile.open("privatekey.priv");
        signatureFile.open("signatures.sig");
        
        for(i=0; i<5000; i++){
            GenerateKeyPair(ctx, TnxKeys.privateKey, TnxKeys.privKey, TnxKeys.publicKey, TnxKeys.publicKey_serilized, TnxKeys.pubKey, options.isverbose);
        
            privatekeyFile << TnxKeys.privKey << std::endl;
            publickeyFile << TnxKeys.pubKey << std::endl;
            ...
            // unsigned char PublicKeyAsMSG [PUBLIC_KEY_SERILIZED_SIZE];
        
            // HexStrToUchar(PublicKeyAsMSG, TnxKeys.pubKey.c_str(), (size_t)PUBLIC_KEY_SERILIZED_SIZE);

            // std::cout << "PublicKey as Message : " << PublicKeyAsMSG << std::endl; 
            //unsigned char msg[1] = {'a'};

            std::string msg = TnxKeys.pubKey; //"a";
            message_hash_str = sha256Data(msg);
            HexStrToUchar(message_hash_char, message_hash_str.c_str(), (size_t)HASH_SHA256_SIZE);
            CHECK(SECP256K1_API::secp256k1_ecdsa_sign(ctx, &signature, message_hash_char /*TnxKeys.publicKey_serilized*/, TnxKeys.privateKey, NULL, NULL) == 1); //make signature
            CHECK(SECP256K1_API::secp256k1_ecdsa_signature_serialize_compact(ctx, signature_serilized, &signature) == 1);
            signature_serilized_str = UcharToHexStr(signature_serilized, SIGNATURE_SERILIZED_SIZE);

            signatureFile << signature_serilized_str << std::endl;
        }
