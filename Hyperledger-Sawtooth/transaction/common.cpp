...
std::string sha256IPFSData(std::string data)
{

    /////////////////////////////////////////////
    //Create a SHA-256 data Hash for IPFS system
    std::vector<uint8_t> message_vect(data.begin(), data.end());
    std::string digest_hex;
    HexEncoder encoder(new StringSink(digest_hex), false);
    std::string digest;
...
std::string sha256IPFSDataFile(std::string  message, std::string file_name)
{

    //std::ofstream measurementsFile;
    //measurementsFile.open(file_name, std::ofstream::out | std::ofstream::app);


    //appel de unixfs
    //std::string str = byteToUnixfs(file_name);
    //std::cout << "after read foo " << std::endl;

    // Unixfs creation the byteToUnixfs() is deprecated
    unixfs::element::pb::Element *myElement;
    myElement = new unixfs::element::pb::Element;

    myElement->mutable_dataelement()->set_data(message);
    myElement->mutable_dataelement()->set_type(unixfs::pb::Data_DataType_File);
    myElement->mutable_dataelement()->set_filesize(message.length());
    
    std::string str = myElement->SerializePartialAsString();

    //std::cout << "unixfs:" << str << std::endl;
    std::vector<uint8_t> message_vect(str.begin(), str.end());
    //
...
std::string byteToUnixfs(std::string file_name)
{
    ...
    myElement->mutable_dataelement()->set_data(message_str);
    myElement->mutable_dataelement()->set_type(unixfs::pb::Data_DataType_File);
    myElement->mutable_dataelement()->set_filesize(message_str.length());

    // std::ofstream myfile;
    // myfile.open("out.raw");
    // myfile << myElement->SerializePartialAsString();
    // myfile.close();


    //####################################  With MerkleDAG ############################################
    // ::unixfs::pb::Data *Data;
    // Data = new ::unixfs::pb::Data;
    // merkledag::pb::PBNode *pbNode;
    // pbNode = new merkledag::pb::PBNode;

    // pbNode->set_data(message_str);
    // std::string dag_string = pbNode->SerializePartialAsString(); //metadata.SerializePartialAsString(); //pbNode->GetMetadata()->SerializePartialAsString();
    

    //::google::protobuf::io::CodedInputStream codedInput; // = pbNode->SerializePartialAsString().c_str();

    //Data->MergeFrom(pbNode)


    
    // Data->set_data(dag_string);
    // Data->set_type(unixfs::pb::Data_DataType_File);
    // Data->set_filesize(dag_string.length());

    // std::ofstream myfile;
    // myfile.open("out.raw");
    // myfile << Data->SerializePartialAsString();
    // myfile.close();

    return myElement->SerializePartialAsString();
}

std::string creatIpfsHash(std::string message, std::string fileName){
    ...
    message_hash_str = sha256IPFSDataFile(message, fileName);
